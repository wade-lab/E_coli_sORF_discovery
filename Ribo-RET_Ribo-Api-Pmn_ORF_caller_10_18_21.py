...
filepath = input("Enter the file path for all the files\n")
input_genome_sequence = filepath + input("Enter the filename for the BL21 genome .fasta file (no gaps between lines)\n")
input_BL21_annotation_file = filepath + input("Enter the filename for the BL21 genes .gff file\n")
input_ret_normalized_gff = filepath + input("Enter the filename for the normalized Ret .gff file\n")
input_apipmn_normalized_gff = filepath + input("Enter the filename for the normalized Api-PMN .gff file\n")
RNAfold_path = '"' + input("Enter the filepath where RNAfold.exe is located\n") + 'RNAfold.exe' + '"'


#/Users/jtw03/Documents/My papers/Api-Ret/files for ORF caller/
#/Users/jtw03/AppData/Local/ViennaRNA Package/

#read in the genome sequence from a .fna file
f = open(input_genome_sequence, 'r')
...
number_of_cycles = 1 + int(input("How many cycles of randomization? [recommended: 10]\n"))
...
min_fold = int(input("How many fold higher must the peak be than the local background? [recommended: 10]\n"))
span = int(input("How many nt on either side of the peak contribute to the local background? [recommended: 50]\n"))

#this is an RPM threshold that Ribo-RET or Ribo-ApiPmn data must exceed for positions to be called as peaks
threshold = float(input("What is the RPM threshold? [recommended: 0.5]\n"))

#peaks must have the highest (or joint highest) coverage in a range +/- "window" nt relative to the peak
window = int(input("How many nt on either side of the peak must have equal or lower coverage? [recommended: 10]\n"))
...
with open(input_BL21_annotation_file, 'r') as BL21gff:
...
for count in range(0, number_of_cycles):
    ...
    plus_dictionary_ret, plus_coord_selected_ret, minus_dictionary_ret, minus_coord_selected_ret = coord_cov_selected(len_of_genome, threshold, window, span, min_fold, input_ret_normalized_gff)
    ...
    plus_dictionary_api, plus_coord_selected_api_before_filtering, minus_dictionary_api, minus_coord_selected_api_before_filtering = coord_cov_selected(len_of_genome,threshold, window, span, min_fold, input_apipmn_normalized_gff)
...
with open(filepath + 'Summary_' + date + '.txt', 'w') as summary:
    ...
    summary.write('Span (must be x-fold above the mean value at positions this distance upstream/downstream) = ' + str(span) + '\n')
